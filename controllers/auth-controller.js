...
const { sendMail, getPasswordResetTemplate } = require("../utils/mail");
...
const addGraphWatchListSupervisor = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error adding graphwl", error: error.message });
  }
};
...
const addTagnamesToTheEmployee = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
...
const forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    const [admin, employee, supervisor, manager] = await Promise.all([
      Admin.findOne({ email }),
    ...
    // Generate and persist reset token (DB stores hashed token + expiry)
    ...
    const resetUrl = `${baseUrl}/reset-password/${userType}/${encodeURIComponent(
      resetToken
    )}`;
...
const resetPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!resetToken) return next(new ErrorResponse("Invalid reset token", 400));
  if (!userType) return next(new ErrorResponse("User type is required", 400));
  if (!password || !confirmPassword) {
    return next(
      new ErrorResponse("Password and confirm password are required", 400)
    );
  }
  if (password !== confirmPassword) {
  ...
  // Assign plain password; let pre-save hook hash it exactly once
  user.password = password;
  user.resetPasswordToken = undefined;
  user.resetPasswordExpire = undefined;
  await user.save();

  // Issue auth token synchronously
  let token;
